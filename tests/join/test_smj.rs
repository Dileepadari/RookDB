...
fn test_smj_produces_results() {
    ...
    // Expected Output:
    // - 5 students, each matching exactly one course.
    assert_eq!(result.tuples.len(), 5, "Each student matches exactly one course");
}

#[test]
fn test_smj_left_outer_join() {
    let _guard = test_lock().lock().unwrap();
    let db = setup_test_db();
    let catalog = load_catalog();
    ...
    let result = smj.execute(&db, &catalog).unwrap();
    // Expected Output:
    // - 5 students match. (If a student didn't match, it would be added with NULLs, but all 5 match).
    assert_eq!(result.tuples.len(), 5, "Left outer join should produce 5 tuples");
}

#[test]
fn test_smj_right_outer_join() {
    let _guard = test_lock().lock().unwrap();
    let db = setup_test_db();
    let catalog = load_catalog();

    let smj = SMJExecutor {
        left_table: "courses".to_string(),
        right_table: "students".to_string(),
        conditions: vec![JoinCondition {
            left_table: "courses".to_string(),
            left_col: "cid".to_string(),
            operator: JoinOp::Eq,
            right_table: "students".to_string(),
            right_col: "course_id".to_string(),
        }],
    ...
    let result = smj.execute(&db, &catalog).unwrap();
    // Expected Output:
    // - All 5 students match to a course.
    // - If any student had an unmatched course_id, it would be output, but here all match.
    assert_eq!(result.tuples.len(), 5, "Right outer join should produce 5 tuples");
}

#[test]
fn test_smj_full_outer_join() {
    let _guard = test_lock().lock().unwrap();
    let db = setup_test_db();
    let catalog = load_catalog();
    ...
    // Expected Output:
    // - 5 students match.
    // - Any unmatched courses would be added, but all 3 courses have matching students.
    assert_eq!(result.tuples.len(), 5, "Full outer join should produce 5 tuples");
}
